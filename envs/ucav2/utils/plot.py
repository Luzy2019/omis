...
def plot_3d_trajectories(self_pos, oppo_pos, fire, lock, dir, file_name):
    ...
    ax.plot(
        ...
        zs=min_z - 1000,
        zdir="z",
        color=self_colors[num_points // 2],
        linestyle="--",
        linewidth=2,
        alpha=1,
    )
    ax.plot(
        ...
        zs=min_z - 1000,
        zdir="z",
        color=oppo_colors[num_points // 2],
        linestyle="--",
        linewidth=2,
        alpha=1,
    )
...
def plot_distance(distance, lock, missile, fire, dir, file_name):
    ...
    # 绘制 `distance` 曲线
    plt.plot(
        distance, color="blue", linewidth=3, marker="None", zorder=2
    )  # 用'None'禁用点的绘制
    ...
    for j in range(len(missile)):
        plt.axvspan(
            missile[j], missile[j] + 1, facecolor="#ACD8E6", alpha=0.6, zorder=1
        )
    ...
    for fire_step in fire:
        ...
        plt.scatter(
            fire_step, distance[fire_step], color="red", s=60, marker="^", zorder=3
        )
...
def plot_2d_trajectories(ally_pos, enemy_pos, save_path=None):
    ...
    # 绘制友军和敌军轨迹
    plt.plot(
        ally_x, ally_z, "b-o", label="Ally Trajectory", markersize=1
    )  # 蓝色线表示友军
    plt.plot(
        enemy_x, enemy_z, "r-o", label="Enemy Trajectory", markersize=1
    )  # 红色线表示敌军
